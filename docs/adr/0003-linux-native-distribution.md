...
`gyazoctl` の native executable を macOS に加えて Linux arm64/amd64 に配布する。`gyazoctl` は npm dependencies を含むため `scriptc --dynamic` が必要だが、dynamic build は build host と同じ OS/architecture の executable だけを生成できる。

## Decision

- GitHub-hosted `ubuntu-24.04` と `ubuntu-24.04-arm` runner で Linux amd64/arm64 を host-native build する。
